...
export type LaunchpadTimeLineProps = {
...
export default function LaunchpadTimeLine({
  ...
  const { startTime, endTime } = useLaunchpadByAddress(launchpadAddress)

  useInterval(() => setCurrent(Date.now()), 1000)

  const start = startTime.toNumber() * 1000
  const startDuration = dayjs.duration(start - current)
  const end = endTime.toNumber() * 1000
