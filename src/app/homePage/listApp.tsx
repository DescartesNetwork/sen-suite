...
const App = ({ route, name, icon }: AppProps) => {
  return (
    <div className="center gap-2">
      <Link
        href={route}
        className="center rounded-full h-28 w-28 md:h-48 md:w-48 p-6 bg-[--opaline] hover:border-accent border-2"
      >
        <Image src={icon} alt="" />
      </Link>
...
export default function ListApp() {
  ...
  return (
    <div ref={listAppRef} className="list-app gap-10">
      <div className="flex flex-row justify-center gap-16 top-app">
        {TOP_APPS.map(({ route, name, icon }) => (
          <div key={route}>
            <App route={route} name={name} icon={icon} />
          </div>
        ))}
      </div>
      <div className="center gap-4">
        <h3 className="title-apps text-[--opaline-content]">
          Let&apos;s explore Sen Suite
        </h3>
        <p className="desc-apps opacity-60 text-[--opaline-content]">
          The limit breaker for projects on Solana.
        </p>
      </div>
      <div className="flex flex-row justify-center gap-16 bottom-app">
      ...
      {listAppRef.current && <ElementIObs querySelector={listAppRef.current} />}
    </div>
  )
}
