'use client'

export default function Description() {
  return (
    <div className="list-app relative">
      <div className="sticky pos-center top-0 left-0 h-[100dvh] w-full gap-10 bg-center bg-no-repeat bg-cover bg-[url('/apps-bg-light.png')] dark:bg-[url('/apps-bg-dark.png')] p-4">
        <h3 className="w-full max-w-[1024px] leading-snug">
          Unlock your project&apos;s full potential with Sentre—Offering swap,
          airdrop tool, launchpad, and more. Begin scaling your decentralized
          journey with us today.
        </h3>
      </div>
      <div className="h-[100vh] w-full" />
    </div>
  )
}
